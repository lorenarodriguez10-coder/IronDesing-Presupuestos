...
async function cambiarEstado(id, nuevoEstado){
  ...
  if(idx === -1) return;
  const registro = state.presupuestos[idx];
  const estadoAnterior = registro.estado || 'pendiente';
  if(estadoAnterior === nuevoEstado) return;

  if(nuevoEstado === 'aceptado' && estadoAnterior !== 'aceptado'){
    await aplicarConsumoInventario(registro);
    showToast('Stock descontado del inventario');
  } else if(estadoAnterior === 'aceptado' && nuevoEstado !== 'aceptado'){
    await revertirConsumoInventario(registro);
    showToast('Se restituyó el stock consumido');
  }
...
function deletePresupuesto(id){
  showConfirm('¿Eliminar este presupuesto del historial?', async ()=>{
    const registro = state.presupuestos.find(p=>p.id===id);
    if(registro && registro.inventarioAplicado){
      await revertirConsumoInventario(registro);
    }
